...
import {
  KILL_REWARD,
  ROUND_WIN_REWARD,
  SPIKE_DETONATION_BONUS,
  SPIKE_PLANT_BONUS,
  clampMoney,
  getLossBonus,
} from './economy';
import { SpikeState, createSpikeState, updateSpikeTimers } from './spike';
import { HitZone, KNIFE, WEAPONS_BY_ID, Weapon } from './weapons';
...
export type RoundEndReason = 'elimination' | 'spike_defused' | 'spike_detonated' | 'time_expired';
...
export interface RoundManagerConfig {
  ...
  /** Défaut 100s (temps max pour poser la spike). */
...
export function createMatchState(config: RoundManagerConfig, initialSpikeCarrierId: string, startTick = 0): MatchState {
  return {
    phase: 'buy',
    roundNumber: 1,
    ...
    spike: createSpikeState(initialSpikeCarrierId),
    teamStats: {
...
export function startNewRound(matchState: MatchState, options: StartRoundOptions): MatchState {
  const { entityManager, eventBus, tick, spawns, initialSpikeCarrierId } = options;
...
function checkRoundEndCondition(matchState: MatchState, context: RoundContext, config: RoundManagerConfig): RoundResult | null {
  ...
  if (matchState.spike.status === 'defused') return { winner: config.defenderTeam, reason: 'spike_defused' };
  if (matchState.spike.status === 'detonated') return { winner: config.attackerTeam, reason: 'spike_detonated' };

  if (matchState.spike.status === 'carried') {
  ...
  // Spike planted/defusing : pas de limite de temps propre au round, le timer de détonation gère déjà la fin.
 ...
 * bonus spike). Les récompenses de kill sont créditées en direct pendant le
 * round (voir `createValorantOnTick`), pas ici.
 */
export function applyRoundEndEconomy(
  ...
  const spikeWasPlanted = matchState.spike.plantedAt !== null;

  entityManager.getAllEntities().forEach((entity) => {
    const isWinner = entity.team === result.winner;
    let reward = isWinner ? ROUND_WIN_REWARD : lossBonus;

    if (!isWinner && entity.team === config.attackerTeam && spikeWasPlanted && result.reason !== 'spike_detonated') {
      reward += SPIKE_PLANT_BONUS;
    }
    if (isWinner && result.reason === 'spike_detonated') {
      reward += SPIKE_DETONATION_BONUS;
    }
...
function resolveShot(
  ...
  const weapon: Weapon = (shooter.equippedWeaponId && WEAPONS_BY_ID[shooter.equippedWeaponId]) || KNIFE;
  ...
  const backstab = weapon.id === 'knife' && isBehindTarget(target, shooter.position);
...
/**
 * Compose le `onTick` complet des règles Valorant (gunplay + économie + round + spike) :
...
export function createValorantOnTick(
  ...
  return (context: TickContext) => {
    ...
    if (matchState.phase === 'active') {
      ...
      matchState = { ...matchState, spike: updateSpikeTimers(matchState.spike, tick, eventBus) };
    }
